          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {recommended.map((prog, index) => (
                <div
                  key={prog.id}
                  className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl p-5 shadow-lg border border-slate-200 dark:border-slate-800 space-y-3 flex flex-col justify-between"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="bg-emerald-100 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 text-[11px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-emerald-200 dark:border-emerald-800/50">
                        <CheckCircle2 className="w-3 h-3" /> {96 - index * 4}% Match
                      </span>
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">{prog.level}</span>
                    </div>

                    <h5 className="font-bold text-sm text-slate-900 dark:text-white leading-snug">{prog.title}</h5>
                    <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2">{prog.summary}</p>
                    <div className="text-xs font-semibold text-lincoln dark:text-red-400">{prog.tuition} • {prog.duration}</div>
                  </div>

                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <Link
                      to={`/programs/${prog.id}`}
                      className="text-xs font-bold text-lincoln dark:text-red-400 hover:underline flex items-center gap-1"
                    >
